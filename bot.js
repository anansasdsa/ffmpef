...
client.user.setGame(`!         iTzAnan_Over#0001 :مصمم البوت `,"http://twitch.tv/S-F")
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
member.addRole(member.guild.roles.find('name','CO'));
});
client.on('guildMemberAdd', member => {
        ...
        fs.readFile(`${w[Math.floor(Math.random() * w.length)]}`, function (err, Background) {
            ...
            ctx.drawImage(ground, 0, 0, 400, 200);
                ...
                let Avatar = Canvas.Image;
                              ...
                              ctx.stroke();
                                 ctx.clip();
                                 ctx.drawImage(ava, 20, 38, 128, 126);  
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("479240245691416607");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("485006927244296203");
    if (!channel) {
  ...
  client.on("message", message => {
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('⚠ | **ليس لديك صلاحيات**');
      ...
      message.channel.sendMessage("", {embed: {
        ...
        footer: {
          text: "capricorn" // غير هنا حط اسم البوت
        }
